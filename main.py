...
# In case you want to group by ALL THE THINGS
    full_tbl = pd.merge(pd.merge(movies_tbl[["MovieID", "Genres", "ReleaseYear"]], ratings_tbl[["MovieID", "UserID", "Rating", "Year", "Month", "Day", "Weekday", "Time"]], "inner", on="MovieID"), users_tbl[["UserID", "Age", "Gender", "Occupation", "StateCode"]], "inner", on="UserID") #let's not look at incomplete data since there's very little of it
...
#%%
    ...
    </html>'''.format(nreviews=str(n_reviews), nusers=str(n_users), nmovies=str(n_movies), usersmap=usersmap_url, reviewsG=reviewsG_url, \
        ages=ages_url, occupations=occupations_url, genres=genres_url, genreratings=genreratings_url,ratingsplot=ratingsplot_url,\
            ratingG=ratingsG_url, ratingsAvg=ratingsAvg_url, summary1=summary_table_1, summary2=summary_table_2, \
                summary3=summary_table_3, summary4=summary_table_4, summary5= summary_table_5)
